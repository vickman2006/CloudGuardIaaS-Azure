...
variable "disk_size" {
  description = "Storage data disk size (GB). Select a number between 100 and 3995"
  type        = string
}
...
variable "admin_shell" {
  description = "The admin shell to configure on machine for the first time"
  type        = string
  default     = "/etc/cli.sh"
}
...
variable "frontend_1_subnet_prefix" {
  description = "Address prefix to be used for network frontend 1 subnet"
  type        = string
  default     = "10.0.0.0/24"
}

variable "frontend_2_subnet_prefix" {
  description = "Address prefix to be used for network frontend 2 subnet"
  type        = string
  default     = "10.0.1.0/24"
}
...
locals {
  ...
  regex_management_GUI_client_network = regex(local.regex_valid_network_cidr, var.management_GUI_client_network) == var.management_GUI_client_network ? 0 : "Variable [management_GUI_client_network] must be a valid IPv4 network CIDR."
}
